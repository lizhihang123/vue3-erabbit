<template>
  <HomePanel title="最新专题">
    <template v-slot:right><XtxMore /></template>
      <div ref="homeSpecial" style="position: relative; height: 403px">
            <Transition name="fade">
                <ul class="special-list"  v-if="list">
                  <li class="special-item" v-for="item in list" :key="item.id">
                    <RouterLink to="/">
                    <img v-lazy="item.cover" alt class="special-img" />
                    <div class="meta">
                      <p class="title">
            ...
            <HomeSpeSkeleton bg="#f0f9f4" v-else />
            </Transition>
      </div>
  </HomePanel>
</template>
...
import HomeSpeSkeleton from './home-speskeleton'
import { findSpecial } from '@/api/home'
// import { ref } from 'vue'
import { useLazyData } from '@/hooks'
export default {
  name: 'HomeSpecial',
  components: { HomePanel, HomeSpeSkeleton },
  setup() {
    // const list = ref([])
